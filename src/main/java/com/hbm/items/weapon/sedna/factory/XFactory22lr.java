...
public class XFactory22lr {
    ...
    public static void init() {
        SpentCasing casing22 = new SpentCasing(SpentCasing.CasingType.STRAIGHT).setColor(SpentCasing.COLOR_CASE_BRASS).setScale(0.5F);
        p22_sp = new BulletConfig().setItem(GunFactory.EnumAmmo.P22_SP).setCasing(ItemEnums.EnumCasingType.SMALL, 24).setKnockback(0F)
                .setCasing(casing22.clone().register("p22"));
        p22_fmj = new BulletConfig().setItem(GunFactory.EnumAmmo.P22_FMJ).setCasing(ItemEnums.EnumCasingType.SMALL, 24).setKnockback(0F).setDamage(0.8F).setThresholdNegation(1F).setArmorPiercing(0.1F)
