...
public class XFactory9mm {
    ...
    public static void init() {
        SpentCasing casing9 = new SpentCasing(SpentCasing.CasingType.STRAIGHT).setColor(SpentCasing.COLOR_CASE_BRASS).setScale(1F, 1F, 0.75F);
        p9_sp = new BulletConfig().setItem(GunFactory.EnumAmmo.P9_SP).setCasing(ItemEnums.EnumCasingType.SMALL, 12)
                .setCasing(casing9.clone().register("p9"));
        p9_fmj = new BulletConfig().setItem(GunFactory.EnumAmmo.P9_FMJ).setCasing(ItemEnums.EnumCasingType.SMALL, 12).setDamage(0.8F).setThresholdNegation(2F).setArmorPiercing(0.1F)
    ...
    public static BiConsumer<ItemStack, ItemGunBaseNT.LambdaContext> LAMBDA_RECOIL_LAG = (stack, ctx) -> ItemGunBaseNT.setupRecoil(5, (float) (ctx.getPlayer().getRNG().nextGaussian() * 1.5));

    public static BiConsumer<ItemStack, ItemGunBaseNT.LambdaContext> LAMBDA_RECOIL_UZI = (stack, ctx) -> ItemGunBaseNT.setupRecoil(1, (float) (ctx.getPlayer().getRNG().nextGaussian() * 0.25));

    public static BiConsumer<ItemStack, ItemGunBaseNT.LambdaContext> LAMBDA_SECOND_UZI = (stack, ctx) -> {
        ...
        GunStateDecider.deciderAutoRefire(stack, ctx, lastState, 0, index, () -> ItemGunBaseNT.getSecondary(stack, index) && ItemGunBaseNT.getMode(stack, ctx.configIndex) == 0);
    };
    ...
    @SuppressWarnings("incomplete-switch") public static BiFunction<ItemStack, HbmAnimationsSedna.GunAnimation, BusAnimationSedna> LAMBDA_GREASEGUN_ANIMS = (stack, type) -> {
        switch (type) {
            case EQUIP -> {
    ...
    @SuppressWarnings("incomplete-switch") public static BiFunction<ItemStack, HbmAnimationsSedna.GunAnimation, BusAnimationSedna> LAMBDA_LAG_ANIMS = (stack, type) -> switch (type) {
    ...
    @SuppressWarnings("incomplete-switch") public static BiFunction<ItemStack, HbmAnimationsSedna.GunAnimation, BusAnimationSedna> LAMBDA_UZI_ANIMS = (stack, type) -> {
        switch (type) {
            case EQUIP -> {
                return new BusAnimationSedna()
                        .addBus("EQUIP", new BusAnimationSequenceSedna().addPos(80, 0, 0, 0).addPos(80, 0, 0, 500).addPos(0, 0, 0, 500, IType.SIN_FULL))
