package com.hbm.inventory.gui;

import com.hbm.Tags;
import com.hbm.packet.PacketDispatcher;
import com.hbm.packet.toserver.NBTControlPacket;
import com.hbm.tileentity.network.TileEntityRadioTorchLogic;
import com.hbm.util.I18nUtil;
import com.hbm.util.SoundUtil;
import net.minecraft.client.Minecraft;
import net.minecraft.client.gui.GuiScreen;
import net.minecraft.client.gui.GuiTextField;
import net.minecraft.client.renderer.GlStateManager;
import net.minecraft.nbt.NBTTagCompound;
import net.minecraft.util.ResourceLocation;
import org.lwjgl.input.Keyboard;
import org.lwjgl.input.Mouse;

import java.io.IOException;
import java.util.Arrays;

import static com.hbm.tileentity.network.TileEntityRadioTorchLogic.MAPPING_SIZE;

public class GUIScreenRadioTorchLogic extends GuiScreen {

    protected static final ResourceLocation texture = new ResourceLocation(Tags.MODID + ":textures/gui/machine/gui_rtty_logic_receiver.png");

    protected TileEntityRadioTorchLogic logic;
    protected GuiTextField frequency;
    protected GuiTextField[] map;
    protected int[] conditions; //so the 'save settings' paradigm applies to the conditions, too

    protected static final int xSize = 256;
    protected static final int ySize = 204;
    protected int guiLeft;
    protected int guiTop;
    private static final int TEXT_COLOR = 0x00FF00;
    private static final int DISABLED_TEXT_COLOR = 0x00FF00;

    public GUIScreenRadioTorchLogic(TileEntityRadioTorchLogic logic) {
        this.logic = logic;
    }

    @Override
    public void initGui() {
        super.initGui();
        this.guiLeft = (this.width - xSize) / 2;
        this.guiTop = (this.height - ySize) / 2;

        Keyboard.enableRepeatEvents(true);

        int oX = 4;
        int oY = 4;

        this.frequency = new GuiTextField(0, this.fontRenderer, guiLeft + 25 + oX, guiTop + 17 + oY, 90 - oX * 2, 14);
        this.frequency.setTextColor(TEXT_COLOR);
        this.frequency.setDisabledTextColour(DISABLED_TEXT_COLOR);
        this.frequency.setEnableBackgroundDrawing(false);
        this.frequency.setMaxStringLength(10);
        this.frequency.setText(logic.channel == null ? "" : logic.channel);

        this.map = new GuiTextField[MAPPING_SIZE];
        this.conditions = new int[MAPPING_SIZE];

        for (int i = 0; i < MAPPING_SIZE; i++) {
            this.map[i] = new GuiTextField(i + 1, this.fontRenderer, guiLeft + 7 + (130 * (i / 8)) + oX + 18, guiTop + 53 + (18 * (i % 8)) + oY, 54 - oX * 2, 14);
            this.map[i].setTextColor(TEXT_COLOR);
            this.map[i].setDisabledTextColour(DISABLED_TEXT_COLOR);
            this.map[i].setEnableBackgroundDrawing(false);
            this.map[i].setMaxStringLength(15);
            this.map[i].setText(logic.mapping[i] == null ? "" : logic.mapping[i]);

            this.conditions[i] = logic.conditions[i];
        }
    }

    @Override
    public void drawScreen(int x, int y, float f) {
        this.drawDefaultBackground();
        this.drawGuiContainerBackgroundLayer(f, x, y);
        GlStateManager.disableLighting();
        this.drawGuiContainerForegroundLayer(x, y);
        GlStateManager.enableLighting();
        //easy selection
        if (guiLeft > x && guiLeft + xSize <= x && guiTop > y && guiTop + ySize <= y) return;

        if (!Mouse.isButtonDown(0) && !Mouse.isButtonDown(1) && Mouse.next()) {
            for (int j = 0; j < MAPPING_SIZE; j++) {
                if (guiLeft + 7 + (130 * (j / 8)) <= x && guiLeft + 7 + 18 + (130 * (j / 8)) > x && guiTop + 53 + (18 * (j % 8)) <= y && guiTop + 53 + 18 + (18 * (j % 8)) > y) {
                    int scroll = Mouse.getEventDWheel();

                    if (scroll > 0) this.conditions[j] = (this.conditions[j] + 1) % 10;
                    if (scroll < 0) this.conditions[j] = (this.conditions[j] + 9) % 10;
                    return;
                }
            }
        }
    }

    @Override
    public boolean doesGuiPauseGame() {
        return false;
    }

    public void drawCustomInfoStat(int mouseX, int mouseY, int x, int y, int width, int height, int tPosX, int tPosY, String[] text) {
        if (x <= mouseX && x + width > mouseX && y < mouseY && y + height >= mouseY) {
            this.drawHoveringText(Arrays.asList(text), tPosX, tPosY);
        }
    }

    private void drawGuiContainerForegroundLayer(int x, int y) {
        String name = I18nUtil.resolveKey("container.rttyLogic");
        this.fontRenderer.drawString(name, this.guiLeft + xSize / 2 - this.fontRenderer.getStringWidth(name) / 2, this.guiTop + 6, 4210752);
        //TODO add localization for *every* RTTY
        drawCustomInfoStat(x, y, guiLeft + 137, guiTop + 17, 18, 18, x, y, new String[]{logic.descending ? "Descending Order" : "Ascending Order"});
        drawCustomInfoStat(x, y, guiLeft + 173, guiTop + 17, 18, 18, x, y, new String[]{logic.polling ? "Polling" : "State Change"});
        drawCustomInfoStat(x, y, guiLeft + 209, guiTop + 17, 18, 18, x, y, new String[]{"Save Settings"});
        for (int j = 0; j < MAPPING_SIZE; j++) {
            if (guiLeft + 7 + (130 * (j / 8)) <= x && guiLeft + 7 + 18 + (130 * (j / 8)) > x && guiTop + 53 + (18 * (j % 8)) <= y && guiTop + 53 + 18 + (18 * (j % 8)) > y) {
                drawCustomInfoStat(x, y, guiLeft + 7 + (130 * (j / 8)), guiTop + 53 + (18 * (j % 8)), 18, 18, x, y, new String[]{I18nUtil.resolveKey("desc.gui.rttyLogic.cond" + this.conditions[j])});
                break;
            }
        }
    }

    private void drawGuiContainerBackgroundLayer(float f, int mouseX, int mouseY) {
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        Minecraft.getMinecraft().getTextureManager().bindTexture(texture);
        drawTexturedModalRect(guiLeft, guiTop, 0, 0, xSize, ySize);
        if (logic.descending) drawTexturedModalRect(guiLeft + 137, guiTop + 17, 0, 204, 18, 18);
        if (logic.polling) drawTexturedModalRect(guiLeft + 173, guiTop + 17, 0, 222, 18, 18);

        for (int i = 0; i < MAPPING_SIZE; i++) {
            if (logic.mapping[i].isEmpty()) {
                if (this.conditions[i] != 0)
                    drawTexturedModalRect(guiLeft + 7 + (130 * (i / 8)), guiTop + 53 + (18 * (i % 8)), 18 + this.conditions[i] * 18, 222, 18, 18);
            } else {
                drawTexturedModalRect(guiLeft + 7 + (130 * (i / 8)), guiTop + 53 + (18 * (i % 8)), 18 + this.conditions[i] * 18, 204, 18, 18);
                drawTexturedModalRect(guiLeft + 85 + (130 * (i / 8)), guiTop + 57 + (18 * (i % 8)), 198, 204, 14, 10);
            }
        }

        for (int i = 0; i < MAPPING_SIZE; i++) this.map[i].drawTextBox();
        this.frequency.drawTextBox();
    }

    @Override
    protected void mouseClicked(int x, int y, int i) throws IOException {
        super.mouseClicked(x, y, i);

        this.frequency.mouseClicked(x, y, i);
        for (int j = 0; j < MAPPING_SIZE; j++) this.map[j].mouseClicked(x, y, i);

        if (guiLeft + 137 <= x && guiLeft + 137 + 18 > x && guiTop + 17 < y && guiTop + 17 + 18 >= y) {
            SoundUtil.playClickSound();
            NBTTagCompound data = new NBTTagCompound();
            data.setBoolean("descending", !logic.descending);
            PacketDispatcher.wrapper.sendToServer(new NBTControlPacket(data, logic.getPos()));
        }

        if (guiLeft + 173 <= x && guiLeft + 173 + 18 > x && guiTop + 17 < y && guiTop + 17 + 18 >= y) {
            SoundUtil.playClickSound();
            NBTTagCompound data = new NBTTagCompound();
            data.setBoolean("polling", !logic.polling);
            PacketDispatcher.wrapper.sendToServer(new NBTControlPacket(data, logic.getPos()));
        }

        if (guiLeft + 209 <= x && guiLeft + 209 + 18 > x && guiTop + 17 < y && guiTop + 17 + 18 >= y) {
            SoundUtil.playClickSound();
            NBTTagCompound data = new NBTTagCompound();
            data.setString("channel", this.frequency.getText());
            for (int j = 0; j < MAPPING_SIZE; j++)
                data.setString("mapping" + j, this.map[j].getText().isEmpty() ? "" : this.map[j].getText());
            for (int j = 0; j < MAPPING_SIZE; j++) data.setInteger("conditions" + j, this.conditions[j]);
            PacketDispatcher.wrapper.sendToServer(new NBTControlPacket(data, logic.getPos()));
        }

        for (int j = 0; j < MAPPING_SIZE; j++) {
            if (guiLeft + 7 + (130 * (j / 8)) <= x && guiLeft + 7 + 18 + (130 * (j / 8)) > x && guiTop + 53 + (18 * (j % 8)) <= y && guiTop + 53 + 18 + (18 * (j % 8)) > y) {
                SoundUtil.playClickSound();
                this.conditions[j] = (this.conditions[j] + 1) % 10;
            }
        }
    }

    @Override
    protected void keyTyped(char c, int i) {
        if (this.frequency.textboxKeyTyped(c, i)) return;

        for (int j = 0; j < MAPPING_SIZE; j++) if (this.map[j].textboxKeyTyped(c, i)) return;

        if (i == 1 || i == this.mc.gameSettings.keyBindInventory.getKeyCode()) {
            this.mc.player.closeScreen();
        }
    }

    @Override
    public void onGuiClosed() {
        Keyboard.enableRepeatEvents(false);
    }
}
