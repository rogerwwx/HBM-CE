...
public class GUIMachineGasCent extends GuiInfoContainer {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
            Tags.MODID + ":textures/gui/processing/gui_centrifuge_gas.png");
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawElectricityInfo(this, mouseX, mouseY, guiLeft + 182, guiTop + 17, 16, 52, gasCent.power, gasCent.maxPower);
        drawInfoTexts(mouseX, mouseY);
        super.renderHoveredToolTip(mouseX, mouseY);
    }

    private void drawTankInfo(int mouseX, int mouseY) {
        String[] inTankInfo = {gasCent.inputTank.getTankType().getName(), gasCent.inputTank.getFill() + " / " + gasCent.inputTank.getMaxFill() + " mB"};

        if (gasCent.inputTank.getTankType().getIfHighSpeed()) {
            inTankInfo[0] = (gasCent.processingSpeed > gasCent.processingSpeed - 70) ? TextFormatting.DARK_RED + inTankInfo[0] : TextFormatting.GOLD + inTankInfo[0];
        }

        String[] outTankInfo = {gasCent.outputTank.getTankType().getName(), gasCent.outputTank.getFill() + " / " + gasCent.outputTank.getMaxFill() + " mB"};

        if (gasCent.outputTank.getTankType().getIfHighSpeed()) {
            outTankInfo[0] = TextFormatting.GOLD + outTankInfo[0];
        }

        drawCustomInfoStat(mouseX, mouseY, guiLeft + 15, guiTop + 15, 24, 55, mouseX, mouseY, inTankInfo);
        drawCustomInfoStat(mouseX, mouseY, guiLeft + 137, guiTop + 15, 25, 55, mouseX, mouseY, outTankInfo);
    }

    private void drawInfoTexts(int mouseX, int mouseY) {
        drawCustomInfoStat(mouseX, mouseY, guiLeft - 12, guiTop + 16, 16, 16, guiLeft - 8, guiTop + 32,
                I18nUtil.resolveKeyArray("desc.gui.gasCent.enrichment"));
        drawCustomInfoStat(mouseX, mouseY, guiLeft - 12, guiTop + 32, 16, 16, guiLeft - 8, guiTop + 48,
    ...
    private void drawEnergyBar() {
        ...
        drawTexturedModalRect(guiLeft + 182, guiTop + 69 - powerHeight, 206, 52 - powerHeight, 16, powerHeight);
    }
    ...
    private void renderTanks() {
        renderTank(guiLeft + 16, guiTop + 16, zLevel, 6, 52, gasCent.inputTank.getFill(),
                gasCent.inputTank.getMaxFill());
        renderTank(guiLeft + 32, guiTop + 16, zLevel, 6, 52, gasCent.inputTank.getFill(),
                gasCent.inputTank.getMaxFill());
        renderTank(guiLeft + 138, guiTop + 16, zLevel, 6, 52, gasCent.outputTank.getFill(),
                gasCent.outputTank.getMaxFill());
        renderTank(guiLeft + 154, guiTop + 16, zLevel, 6, 52, gasCent.outputTank.getFill(),
                gasCent.outputTank.getMaxFill());
    }
