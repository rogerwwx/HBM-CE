...
public class RadioTorchCounter extends RadioTorchBase implements IGUIProvider {
    ...
    public boolean canBlockStay(World world, EnumFacing dir, Block b, BlockPos checkPos, IBlockState checkState) {
        if (b.isSideSolid(checkState, world, checkPos, dir) || (b.isFullCube(checkState) && !b.isAir(checkState, world, checkPos)))
            return true;
    ...
    public Container provideContainer(int ID, EntityPlayer player, World world, int x, int y, int z) {
        return new ContainerCounterTorch(player.inventory, (TileEntityRadioTorchCounter) world.getTileEntity(new BlockPos(x, y, z)));
    }

    @Override
    @SideOnly(Side.CLIENT)
    public GuiScreen provideGUI(int ID, EntityPlayer player, World world, int x, int y, int z) {
        return new GUICounterTorch(player.inventory, (TileEntityRadioTorchCounter) world.getTileEntity(new BlockPos(x, y, z)));
    }
