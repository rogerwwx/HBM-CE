...
public class Spotlight extends Block implements ISpotlight, INBTBlockTransformable, IDynamicModels {
    ...
    public Spotlight(String name, Material mat, int beamLength, LightType type, boolean isOn) {
        ...
        setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.UP).withProperty(BROKEN, false).withProperty(CONN_UP, false).withProperty(CONN_DOWN, false).withProperty(CONN_NORTH, false).withProperty(CONN_SOUTH, false).withProperty(CONN_WEST, false).withProperty(CONN_EAST, false));
    ...
    protected @NotNull BlockStateContainer createBlockState() {
        return new BlockStateContainer(this, FACING, BROKEN, CONN_UP, CONN_DOWN, CONN_NORTH, CONN_SOUTH, CONN_WEST, CONN_EAST);
    }
    ...
    public @NotNull AxisAlignedBB getBoundingBox(@NotNull IBlockState state, @NotNull IBlockAccess source, @NotNull BlockPos pos) {
        ...
        float[] offset = new float[]{0.5F - facing.getXOffset() * (0.5F - bounds[0]), 0.5F - facing.getYOffset() * (0.5F - bounds[1]), 0.5F - facing.getZOffset() * (0.5F - bounds[2])};

        return new AxisAlignedBB(offset[0] - bounds[0], offset[1] - bounds[1], offset[2] - bounds[2], offset[0] + bounds[0], offset[1] + bounds[1], offset[2] + bounds[2]);
    }
    ...
    public @NotNull IBlockState getActualState(IBlockState state, @NotNull IBlockAccess world, BlockPos pos) {
        ...
        return state.withProperty(CONN_UP, cu).withProperty(CONN_DOWN, cd).withProperty(CONN_NORTH, cn).withProperty(CONN_SOUTH, cs).withProperty(CONN_WEST, cw).withProperty(CONN_EAST, ce);
    }
    ...
    private boolean updatePower(World world, BlockPos pos) {
        ...
        if (isOn && isPowered) {
            ...
            world.setBlockState(pos, getOn().getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BROKEN, state.getValue(BROKEN)), 2);
            return true;
        }
    ...
    public void updateTick(@NotNull World world, @NotNull BlockPos pos, @NotNull IBlockState state, @NotNull Random rand) {
        ...
        if (isOn && world.isBlockPowered(pos)) {
            IBlockState newState = getOff().getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BROKEN, state.getValue(BROKEN));
    ...
    private void repair(World world, BlockPos pos) {
        ...
        world.setBlockState(pos, getOn().getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(BROKEN, false), 2);
    ...
    public static void propagateBeam(World world, BlockPos pos, EnumFacing facing, int distance, int meta) {
        distance--;
        ...
        if (!(state.getBlock() instanceof SpotlightBeam)) {
            world.setBlockState(pos, ModBlocks.spotlight_beam.getDefaultState().withProperty(META, meta), 3);
        }
    ...
    public void bakeModel(ModelBakeEvent event) {
        ...
        try {
            model = new HFRWavefrontObject(new ResourceLocation(Tags.MODID, String.format("models/lights/%s.obj", getModel())));
        } catch (Exception _) {
            return;
        }
